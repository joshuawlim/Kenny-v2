"""
Propose Reply capability handler for iMessage messages.

This handler provides functionality to propose replies to iMessage messages,
considering context, attachment content, and conversation history.
"""
...
class ProposeReplyCapabilityHandler(BaseCapabilityHandler):
    """Handler for proposing replies to iMessage messages."""
    
    def __init__(self):
        """Initialize the propose reply capability handler."""
        super().__init__(
            capability="messages.propose_reply",
            description="Propose contextually appropriate replies to iMessage messages",
            input_schema={
                "type": "object",
                "properties": {
                    ...
                    "consider_attachments": {"type": "boolean", "default": True},
                ...
                "properties": {
                    "proposals": {
                        ...
                        "items": {
                            ...
                            "properties": {
                                ...
                                "style": {"type": "string"},
                                "considers_attachments": {"type": "boolean"},
                    ...
                    "original_message": {
                        ...
                        "properties": {
                            ...
                            "attachment_summary": {"type": ["string", "null"]},
    ...
    async def execute(self, parameters: Dict[str, Any]) -> Dict[str, Any]:
        ...
        try:
            ...
            message_data = await self._get_message_data(parameters, imessage_bridge_tool)
            if not message_data:
                return self._get_mock_proposals(parameters)
            
            # Process attachment content if present
            attachment_summary = None
            if parameters.get("consider_attachments", True) and message_data.get("has_attachments"):
                attachment_summary = await self._process_message_attachments(message_data, image_processor_tool)
            
            # Get conversation context
            context_messages = await self._get_conversation_context(
                parameters, message_data, imessage_bridge_tool
            )
            
            # Generate reply proposals
            proposals = await self._generate_reply_proposals(
                message_data, attachment_summary, context_messages, parameters
            )
            ...
            return {
                ...
                "original_message": {
                    ...
                    "has_attachments": bool(message_data.get("has_attachments")),
            ...
            print(f"iMessage propose reply error: {e}")
            return self._get_mock_proposals(parameters)
    ...
    async def _get_message_data(self, parameters: Dict[str, Any], bridge_tool: Any) -> Optional[Dict[str, Any]]:
        ...
        try:
            ...
            if thread_id and message_content:
                return {
                    "id": "provided_content",
                    "thread_id": thread_id,
    ...
    async def _process_message_attachments(self, message_data: Dict[str, Any], image_processor: Any) -> Optional[str]:
        """Process attachment content and return a summary."""
        try:
            if not image_processor or not message_data.get("has_attachments"):
                return None
                ...
                if attachment_type in ["image", "photo"]:
                    ...
                    if local_path:
                        try:
                            ...
                            summary_parts.append("Image content could not be processed")
                elif attachment_type in ["video", "movie"]:
                    summary_parts.append("Video attachment")
                elif attachment_type in ["audio", "voice"]:
                    summary_parts.append("Audio/voice attachment")
                else:
                    filename = attachment.get("filename", f"{attachment_type} file")
                    summary_parts.append(f"Attachment: {filename}")
            
            return "; ".join(summary_parts) if summary_parts else "Attachments processed"
            
        except Exception as e:
            print(f"Attachment processing error: {e}")
            return "Attachment content could not be processed"
    ...
    async def _get_conversation_context(self, parameters: Dict[str, Any], 
        ...
        try:
            ...
            if result.get("result", {}).get("messages"):
    ...
    async def _generate_reply_proposals(self, message_data: Dict[str, Any], attachment_summary: Optional[str],
        ...
        try:
            ...
            message_type = self._classify_message_type(message_content, attachment_summary)
            ...
            proposals.append({
                ...
                "reasoning": reasoning,
                "style": reply_style,
                "considers_attachments": bool(attachment_summary),
            ...
            # Reply 3: Attachment-aware response if attachments are present
            if attachment_summary and max_replies > 2:
                attachment_reply = self._generate_attachment_aware_response(
                    message_content, attachment_summary, reply_style
                )
                proposals.append({
                    "reply_text": attachment_reply,
                    "confidence": 0.7,
                    "reasoning": "Response considering attachment content",
                    "style": reply_style,
                    "considers_attachments": True,
                    "context_used": len(context_messages)
                })
    ...
    def _classify_message_type(self, content: str, attachment_summary: Optional[str]) -> str:
        ...
        # Greeting indicators
        if any(word in content_lower for word in ["hello", "hi", "hey", "good morning", "good evening", "how are you"]):
            return "greeting"
        
        # Information sharing indicators
        if any(word in content_lower for word in ["here is", "here's", "check this", "look at"]) or attachment_summary:
            return "information"
    ...
    def _generate_attachment_aware_response(self, content: str, attachment_summary: str, style: str) -> str:
        """Generate response that acknowledges attachment content."""
        if "image" in attachment_summary.lower():
            if style == "professional":
        ...
        elif "video" in attachment_summary.lower():
            if style == "professional":
                return "Thank you for sharing the video content."
            elif style == "brief":
                return "Nice video!"
            else:  # casual
                return "Cool video, thanks for sending!"
        elif "audio" in attachment_summary.lower() or "voice" in attachment_summary.lower():
            if style == "professional":
                return "Thank you for the audio message."
            elif style == "brief":
                return "Got your voice message!"
            else:  # casual
                return "Thanks for the voice message!"
        else:
            if style == "professional":
                return "Thank you for sharing the attachment."
            elif style == "brief":
                return "Thanks for sharing!"
            else:
    ...
    def _get_mock_proposals(self, parameters: Dict[str, Any]) -> Dict[str, Any]:
        ...
        proposals = [
            {
                ...
                "style": style,
                "considers_attachments": parameters.get("consider_attachments", True),
        ...
        return {
            ...
            "original_message": {
                "content": parameters.get("message_content", "Mock iMessage content"),
                "has_attachments": False,
                "attachment_summary": None,
