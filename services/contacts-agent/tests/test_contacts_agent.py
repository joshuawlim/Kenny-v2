...
class TestContactsAgent:
    ...
    async def test_agent_start_success(self):
        ...
        with patch.object(agent, 'registry_client') as mock_registry:
            ...
            with patch.object(agent, 'health_monitor') as mock_health:
...
class TestContactsBridgeTool:
    ...
    def test_tool_initialization(self):
        ...
        assert tool.description == "Access contact data through the macOS bridge service"
    ...
    async def test_search_contacts_by_email(self):
        ...
        assert len(contacts) == 1
        assert contacts[0]["emails"] == ["john.doe@example.com"]
    ...
    async def test_enrich_contact(self):
        ...
        assert enrichments[0]["type"] == "job_title"
        assert enrichments[0]["value"] == "Software Engineer"
